...
It creates the GUI that we're going to use to preview the live video as well as
...
class GUI(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None):
        # We define the main properties of the window
        QtWidgets.QWidget.__init__(self, parent)
        self.setWindowTitle("Digit Classification (Keras CNN trained with " +
                            "an enlarged MNIST database)")
        ...
        lab9.setStyleSheet("background-color: #7FFFD4; color: #000; " +
                           "font-size: 20px; border: 1px solid black;")
        self.digs.append(lab9)


    def setCamera(self,cam):
        self.cam=cam

    # This function update the GUI for every time the thread change
    def update(self):
        # We get the original image from the camera and display it
        ...
        # We get the transformed image and display it as a grayscale image
        im_prev_trans = self.cam.getImage()[1]
        im_trans = QtGui.QImage(im_prev_trans.data, im_prev_trans.shape[1],
                                im_prev_trans.shape[0],QtGui.QImage.Format_Indexed8)
        ...
        net_out = self.cam.detection(im_trans)
        self.lightON(net_out)

    # This function updates which digit has the "light on" dependeing on the 
    # network output
    def lightON(self,out):
        for dig in self.digs:
            dig.setStyleSheet("background-color: #7FFFD4; color: #000; " +
                              "font-size: 20px; border: 1px solid black;")
            self.digs[out].setStyleSheet("background-color: #FFFF00; " +
